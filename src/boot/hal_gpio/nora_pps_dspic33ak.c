...
/*
 * The resident AK128 bootloader has one deliberately fixed PPS topology:
 * U1TX -> RP50 and U1RX <- RP52.  Keep the generic HAL API and routing order,
 * but compile unused peripheral/RP switch cases out of that ROM image.  The
 * profile is intentionally tied to SONORA_RESIDENT_BOOTLOADER_AK128, derived
 * from the resident build identity and exact device in resident_de_boot_profile.h.
 * Application and larger-device builds retain the complete device-adaptive
 * table below.
 */
...
#if SONORA_RESIDENT_BOOTLOADER_AK128
    (void)rp;
    return false;   /* resident profile exposes no virtual PPS endpoints */
#else

...
/* The AK128 resident image has one console: UART1. Keep the generic device
 * table and all instances for application/AK512 builds, but omit unused UART
 * register and IRQ mappings from its ROM image. */
#if SONORA_RESIDENT_BOOTLOADER_AK128
static const nora_uart_dspic33ak_device_t g_uart_devices[] = {
#if defined(U1CON)
    {
        .present = true,
        .regs = {
...
{
#if SONORA_RESIDENT_BOOTLOADER_AK128
    if (inst != NORA_UART_INST_1 || !g_uart_devices[0].present) {
        return 0;
    }

    return &g_uart_devices[0];
#else

...
#if SONORA_RESIDENT_BOOTLOADER_AK128
    /*
     * This profile does not name a PLL's frequency (see the gate in
     * nora_clock_source_hz), so a PLL-sourced system clock reports 0 here. Left as
     * 0 that would take the paragraph above at its word and skip the check -- and
     * this is the one caller where doing so is NOT safe, because the direction of
     * this call is not always downwards: /1 is the smallest divider, so applying it
     * on a PLL raises Fosc towards the PLL's output. That is the over-clock this
     * check exists for -- gating the PLL readback out entirely was rejected for the
     * same reason.
     *
     * So bound the unknown instead of dropping it. The most any PLL on this part can
     * produce is DSPIC33AK_CLOCK_OUTPUT_MAX_HZ, so substituting it can only make
     * this check refuse MORE often than the true frequency would, never less -- the
     * gate cannot weaken the preflight, only tighten it. The bound is not reported
     * anywhere and does not reach a divisor: it exists for the length of this
     * comparison.
     *
     * The resident does not reach this: it calls nora_clock_switch_source(FRC) first
     * and that confirms FRC before returning OK. This is the belt for the case where
     * that reasoning is wrong, not a path the image is expected to take.
     */
    if (source_hz == 0u) {
        nora_clock_pll_t unknown_parent;

        if (source_parent_pll(source, &unknown_parent)) {
            source_hz = DSPIC33AK_CLOCK_OUTPUT_MAX_HZ;
        }
    }
#endif
    if (source_hz != 0u) {
...
#else
    /*
     * The resident never names a PLL's frequency, so on this profile the two PLL
     * cases fall through to the 0 that the contract above already defines as
     * "unknown" -- and which every caller in this file already handles as such.
     * That drops pll_output_hz() and, with it, the PLL register readback, the PLL
     * source decoder, and the last use of the 64-bit divide helpers in this module.
     *
     * WHY THIS CANNOT CHANGE THE RESIDENT'S BEHAVIOUR. Its whole clock surface is
     * four calls (resident_de_boot_platform.c, resident_de_boot_main.c):
     * nora_clock_switch_source(FRC, 0), nora_clock_dspic33ak_system_divider_set(1),
     * nora_clock_dspic33ak_clkgen_configure(CLKGEN6 and CLKGEN8, {FRC, /1}), and
     * nora_clock_dspic33ak_raw_capture(). Of those:
     *
     *   - the switch and the CLKGEN calls pass a source that is the literal FRC at
     *     the call site, so they reach this function's FRC case and nothing else;
     *   - configure_clkgen() does not consult a frequency at all;
     *   - raw_capture() reads registers without decoding them;
     *   - only nora_clock_dspic33ak_system_divider_set() passes a source READ FROM
     *     HARDWARE -- and it runs after the switch to FRC returned OK, so the part
     *     is on FRC by then.
     *
     * AND IF THAT ORDERING ARGUMENT WERE WRONG the unchecked case would NOT be the
     * safe direction, which is why it is not left unchecked. /1 is the SMALLEST
     * divider, so applying it while the part is on a PLL raises Fosc towards that
     * PLL's output -- exactly the over-clock check_operating_point() exists to
     * refuse. nora_clock_dspic33ak_system_divider_set() therefore BOUNDS the
     * unknown on this profile rather than skipping the check; see the gate
     * there. This function keeps answering 0, because 0 is the honest answer and the
     * one thing it must never do is put a plausible number on a frequency it cannot
     * read.
     *
     * NOT GATED, deliberately: source_parent_pll() and the PLL-ready precondition in
     * nora_clock_switch_source(), and the CLKGEN source decoder. Reset entry may
     * still find the part on a PLL and must still identify that correctly; what this
     * profile gives up is only putting a number on it.
     */
#endif
    default:
        break;
...
static bool system_source_encode(nora_clock_source_t source, uint16_t *encoded)
{
    switch (source) {
#if SONORA_RESIDENT_BOOTLOADER_AK128
    /* The resident only ever switches the system clock to FRC, and the CLKGEN
     * encoder this delegates to is already FRC-only on this profile (see
     * nora_clock_device_encode_clkgen_source()), so the remaining portable
     * sources can no longer succeed past it. Narrowing the filter to match
     * removes their case labels too. The decoders and the operating-point
     * preflight in nora_clock_switch_source() are untouched: reset may still
     * find the part on a non-FRC source and must still identify it correctly. */
    case NORA_CLOCK_SOURCE_FRC:
        break;
#else
    case NORA_CLOCK_SOURCE_FRC:

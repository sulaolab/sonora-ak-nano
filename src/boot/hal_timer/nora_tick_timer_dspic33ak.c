...
/*
 * FIXED PROFILE. The resident bootloader runs from one clock it programs itself and
 * never re-clocks: nora_clock_switch_source(FRC) then the /1 system divider, so
 * Fosc = 8 MHz and Timer1's standard-speed peripheral clock = Fosc / 2 = 4 MHz
 * (resident_de_boot_platform.c RESIDENT_FCY_HZ). Both operands of the period
 * calculation are therefore build-time constants, but the generic path cannot see
 * that: the config struct crosses a translation unit as a pointer, so the search
 * below runs at every boot to re-derive PR1 = 3999 and TCKPS = 1:1.
 *
 * THE HEADER'S PROMISE IS KEPT, NOT DROPPED. nora_tick_timer.h refuses an inexact
 * period rather than rounding it, for the reason written at ERR_INEXACT_PERIOD --
 * every cadence in the tree is milliseconds of this tick. Here that promise is
 * PROVEN at build time by the assertions below instead of being searched for at run
 * time, which is strictly stronger for an image whose clock cannot vary. What the
 * profile gives up is only the ability to be told about it at run time:
 * ERR_INEXACT_PERIOD and ERR_OUT_OF_RANGE become compile errors in this image, so
 * they are unreachable here. Every other status, including the FRC refusal in
 * apply_clock_source() and the argument validation, is unchanged.
 *
 * A caller that asks for a different clock is still refused (ERR_INVALID_ARG) rather
 * than silently given a 4 MHz period -- that check is the one comparison this costs.
 */
#define DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ   UINT32_C(4000000)

/* TCKPS 0b00 = 1:1. The prescaler is not searched, so it is named here. */
#define DSPIC33AK_TICK_TIMER_FIXED_TCKPS    0u

_Static_assert((DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ % DSPIC33AK_TICK_TIMER_HZ) == 0u,
    "ERR_INEXACT_PERIOD at build time: 1.000 ms is not an exact number of ticks at "
    "1:1. Fix the resident clock, not this file -- see nora_tick_timer.h.");
_Static_assert((DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ / DSPIC33AK_TICK_TIMER_HZ) >= 1u,
    "ERR_INEXACT_PERIOD at build time: fewer than one tick per millisecond.");
_Static_assert(((uint64_t)(DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ /
    DSPIC33AK_TICK_TIMER_HZ) - 1u) <= UINT32_MAX,
    "ERR_OUT_OF_RANGE at build time: the 1 ms count does not fit PR1 at 1:1. A "
    "larger prescaler would be needed, which this fixed profile does not select.");

#define DSPIC33AK_TICK_TIMER_FIXED_PR1 \
    ((uint32_t)((DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ / DSPIC33AK_TICK_TIMER_HZ) - 1u))
#else
typedef struct {
    uint16_t divisor;
    uint8_t tckps;
...
#if SONORA_RESIDENT_BOOTLOADER_AK128
/*
 * Fixed-profile derivation: the same validation, then two constants instead of the
 * prescaler search. The argument checks are NOT part of what the profile removes --
 * a wrong priority or an FRC request must still be reported to the caller, and the
 * clock equality below is what keeps "PR1 = 3999" a fact about the argument rather
 * than an assumption about it.
 *
 * Static, one call site: the constants propagate into nora_tick_timer_init()'s
 * register writes, which is the point of the gate.
 */
static nora_tick_timer_status_t calc_period_reg(
    const nora_tick_timer_config_t *config,
    uint32_t *period_reg,
    uint8_t *tckps)
{
    ...
    if ((config == 0) || (period_reg == 0) || (tckps == 0) ||
        (config->timer_clk_hz != DSPIC33AK_TICK_TIMER_FIXED_CLK_HZ) ||
    ...
    *period_reg = DSPIC33AK_TICK_TIMER_FIXED_PR1;
    *tckps = DSPIC33AK_TICK_TIMER_FIXED_TCKPS;

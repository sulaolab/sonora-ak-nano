...
#  error "hal_noinit_ram: reserve the block in the linker script first (KEEP + NOLOAD + a guard filler so the stack cannot be placed there), then #define NORA_NOINIT_RAM_LINKER_RESERVED 1. The attribute alone does NOT protect the range -- see recorded validation part 3."
#endif

...
#define NORA_GPIO_PORT_ROW_NO_ANSEL(L) \
    { &PORT##L, &LAT##L, &TRIS##L, 0, &ODC##L, &CNPU##L, &CNPD##L }
...
{
#if defined(LATA) && defined(ANSELA)
    NORA_GPIO_PORT_ROW(A),     /* [0] PORTA */
#elif defined(LATA)
    NORA_GPIO_PORT_ROW_NO_ANSEL(A),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATB) && defined(ANSELB)
    NORA_GPIO_PORT_ROW(B),     /* [1] PORTB */
#elif defined(LATB)
    NORA_GPIO_PORT_ROW_NO_ANSEL(B),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATC) && defined(ANSELC)
    NORA_GPIO_PORT_ROW(C),     /* [2] PORTC */
#elif defined(LATC)
    NORA_GPIO_PORT_ROW_NO_ANSEL(C),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATD) && defined(ANSELD)
    NORA_GPIO_PORT_ROW(D),     /* [3] PORTD */
#elif defined(LATD)
    NORA_GPIO_PORT_ROW_NO_ANSEL(D),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATE) && defined(ANSELE)
    NORA_GPIO_PORT_ROW(E),     /* [4] PORTE */
#elif defined(LATE)
    NORA_GPIO_PORT_ROW_NO_ANSEL(E),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATF) && defined(ANSELF)
    NORA_GPIO_PORT_ROW(F),     /* [5] PORTF */
#elif defined(LATF)
    NORA_GPIO_PORT_ROW_NO_ANSEL(F),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATG) && defined(ANSELG)
    NORA_GPIO_PORT_ROW(G),     /* [6] PORTG */
#elif defined(LATG)
    NORA_GPIO_PORT_ROW_NO_ANSEL(G),
#else
    NORA_GPIO_PORT_NONE,
#endif
#if defined(LATH) && defined(ANSELH)
    NORA_GPIO_PORT_ROW(H),     /* [7] PORTH */
#elif defined(LATH)
    NORA_GPIO_PORT_ROW_NO_ANSEL(H),
#else
    NORA_GPIO_PORT_NONE,
#endif
};
...
bool nora_gpio_set_analog(nora_gpio_pin_t pin, bool analog)
{
    ...
    if (r->ansel == 0)
    {
        /* This package has no analogue select register for the port. It is
         * permanently digital: accepting the digital request preserves the
         * normal pinmux path; an analogue request is not supportable. */
        return !analog;
    }
    uint32_t mask = nora_gpio_mask(pin);
